...
$subjectTeacherMap=[];
foreach($subjects as $si=>$sub)
    $subjectTeacherMap[$si]=$si; // teacher index matches subject index directly
...
.sig-line{border-top:1.5px solid #94a3b8;width:160px;margin:8px auto 5px;}
.sig-name{font-weight:700;font-size:.8rem;color:var(--text);}
.sig-role{font-size:.68rem;color:var(--muted);margin-top:2px;}
...
@media print{
  #cd-overlay{display:none!important;}
  nav,.tabs,.legend,footer,.btn-print,.btn-back,.summary{display:none!important;}
  #app{display:block!important;}
  .tt-wrap{display:block!important;page-break-after:always;}
...
<div id="app">
<nav>
  <a href="index.html" class="nav-logo">📅 TimeTable</a>
  <div class="nav-actions">
    <a href="index.html" class="btn-back">← New Timetable</a>
      ...
      <table class="pdf-tbl">
        <thead>
          <tr>
            ...
            <?php $li=false;$pn=1; foreach($nlSlots as $si=>$sl): ?>
        ...
        <tbody>
          ...
          <tr>
            ...
            <?php $li3=false; foreach($slots as $si=>$sl):
              if($sl['is_lunch']):
                $li3=true; ?>
...
(function(){
  const ov=document.getElementById('cd-overlay'),
        num=document.getElementById('cd-n'),
        circ=document.getElementById('cd-c'),
        steps=[document.getElementById('s1'),document.getElementById('s2'),document.getElementById('s3')];
  let c=3;
  const CIRC=314;
  function tick(){
    num.textContent=c;
    circ.style.strokeDashoffset=CIRC*(c/3);
    if(c===3)steps[0].classList.add('on');
    if(c===2)steps[1].classList.add('on');
    if(c===1)steps[2].classList.add('on');
    if(c===0){
      ov.classList.add('out');
      setTimeout(()=>{ov.style.display='none';document.getElementById('app').style.display='block';},650);
      return;
    }
